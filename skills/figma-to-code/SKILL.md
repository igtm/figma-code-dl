...
5. **local Figma MCP server を優先**（`http://127.0.0.1:3845/mcp`）。動いていなければ cloud 版にフォールバック。

## 事前確認：Figma ファイルがアクティブタブか？

ローカルの Dev Mode MCP は **Figma Desktop で今アクティブになっているタブのファイル** に対してしか動きません。URL の fileKey は飾りで、active tab が違うファイルだと `get_metadata` / `get_design_context` / `get_variable_defs` のいずれも `No node could be found ...` で失敗します。

なので **MCP を叩く前に必ずユーザーに確認**してください。最初に次のように聞いてから作業に入る：

> Figma Desktop で対象のファイルを開いて、**そのタブをアクティブ（最前面）** にしてください。準備できたら教えてください。

これが終わったら以下のフローへ。Figma Desktop が起動していない／Dev Mode MCP server が OFF の場合も同様に止まるので、ユーザーに案内：

- Figma Desktop を起動
- 対象ファイルを開いてアクティブタブにする
- Figma メニュー → Preferences → "Enable Dev Mode MCP server" が ON
...
2. local MCP 疎通確認 → NG なら cloud。両方 NG なら URL の情報だけで起こす（page 名は `<TBD>`）。
